...
from client_consumer import status_consumer_loop, telemetry_consumer_loop
...
def start_ev():
    ...
    if ev_thread is None or not ev_thread.is_alive():
        ev_thread = threading.Thread(
            ...
            args=(CAR_ID, producer_stop_event, lambda: get_ev_label(CAR_ID)),
            daemon=True
        )
        ev_thread.start()
...
def startup():
    global telemetry_thread, status_thread
