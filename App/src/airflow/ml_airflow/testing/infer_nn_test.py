...
def submit_predict_job(hdfs_url: str, car_ids: list, predict_date: str, user_id: str = "system_admin"):
    """Submit a prediction job to AIAPI."""
    resp = requests.post(
        f"{AIAPI_URL}/api/v1/predict",
        json={
            "HDFS_URL": hdfs_url,
            "car_ids": car_ids,
    ...
    data = resp.json()
    print(f"Prediction job submitted: {data['job_id']}")
    return data["job_id"]
...
def run_inference_pipeline_nn(car_ids=None, predict_date=None):
    if car_ids is None:
        car_ids = ['EV_104', 'EV_105', 'EV_109', 'EV_110']
    if predict_date is None:
        predict_date = 'latest'

    print("=== Submitting prediction job to AIAPI ===")
    ...
    job_id = submit_predict_job(HDFS_URL, car_ids, predict_date, USER_ID)
