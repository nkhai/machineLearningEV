...
def submit_train_job(hdfs_url: str, user_id: str = "system_admin"):
    """Submit a training job to AIAPI."""
    resp = requests.post(
        f"{AIAPI_URL}/api/v1/train",
        json={
            "HDFS_URL": hdfs_url,
            "user_id": user_id,
        },
    ...
    data = resp.json()
    print(f"Training job submitted: {data['job_id']}")
    return data["job_id"]
...
def run_train_pipeline(hdfs_url=None, user_id=None):
    if hdfs_url is None:
        hdfs_url = HDFS_URL
    if user_id is None:
        user_id = USER_ID

    print("=== Submitting training job to AIAPI ===")
    print(f"AIAPI_URL: {AIAPI_URL}")
    print(f"HDFS_URL: {hdfs_url}")
    print(f"USER_ID: {user_id}")

    job_id = submit_train_job(hdfs_url, user_id)
